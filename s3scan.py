...
from s3scan_api import printSocket
...
def scanner(url):
	...
	if globalBaseUrl in url:
		if url not in globalLinkList:
			...
			sys.stdout.write(printSocket("[>]Current webpage: " + url + "\n", "blue"))
			...
			for bucket in re.findall(reg, page_source):
				...
				sys.stdout.write(printSocket("[*]Found " + bucket + "\n", "blue"))
				...
				sys.stdout.write(printSocket("[>]Testing " + bucketName + "\t", "blue"))
				...
				try:
					...
					if readFlag and not writeFlag and not fullControlFlag:
						sys.stdout.write(printSocket("[Insecure - Read]", "fail"))
					elif readFlag and writeFlag and not fullControlFlag:
						sys.stdout.write(printSocket("[Insecure - Read+Write]", "fail"))
					elif fullControlFlag:
						sys.stdout.write(printSocket("[Insecure - Full Control]", "fail"))
					else:
						sys.stdout.write(printSocket("[Not Public]", "green"))
					sys.stdout.write('\n')

				except botocore.exceptions.ClientError as e:
					if e.response["Error"]["Code"] == "NoSuchBucket":
						sys.stdout.write(printSocket("[No Such Bucket. Takeover?]\n", "fail"))
					else:
						sys.stdout.write(printSocket("[" + e.response["Error"]["Code"] + "]\n", "green"))
...
def driver(url):
	...
	if page_source != None:
		...
		sys.stdout.write(printSocket("[x]Empty response. Skipping\n", "warn"))
...
def initiator(globalBaseUrl):
	...
	if True:
		...
		if access_key is None or secret_key is None:
			sys.stdout.write(printWarning("""No access credentials available.
	...
	sys.stdout.write(printSocket("[>]Initiating...", "blue"))
	sys.stdout.write(printSocket("[>]Press Ctrl+C to terminate script", "blue"))
